...
class BlockRendererImpl(override val context: RebuildContextImpl) : BlockRenderer<BlockState, FluidState>() {
    ...
    override fun renderBlock(state: BlockState) {
        ...
        val offsetType = state.offsetType
        if (offsetType != OffsetType.NONE) {
            val hashCode = MathHelper.hashCode(context.blockX, 0, context.blockZ)
            val maxHorizontalOffset = state.block.maxHorizontalModelOffset
            context.renderPosX += ((hashCode and 15L) / 30.0f - 0.25f).coerceIn(-maxHorizontalOffset, maxHorizontalOffset)
            context.renderPosZ += ((hashCode shr 8 and 15L) / 30.0f - 0.25f).coerceIn(-maxHorizontalOffset, maxHorizontalOffset)
            if (offsetType == OffsetType.XYZ) context.renderPosY += ((hashCode shr 4 and 15L) / 15.0f - 1.0f) * state.block.verticalModelOffsetMultiplier
        }
