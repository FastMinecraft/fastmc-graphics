...
import dev.fastmc.graphics.mixin.accessor.AccessorBiomeAccess
import dev.fastmc.graphics.mixin.accessor.AccessorLightStorage
import dev.fastmc.graphics.shared.terrain.WorldSnapshot113
...
class WorldSnapshotImpl(override val context: RebuildContextImpl) :
    ...
    override fun getWorldBrightness(direction: Int, shaded: Boolean): Int {
        val darkened = world.skyProperties.isDarkened
        return if (!shaded) {
    ...
    override fun getColor(x: Int, y: Int, z: Int, colorResolver: ColorResolver): Int {
        val accessor = world.biomeAccess as AccessorBiomeAccess
        return colorResolver.getColor(accessor.type.getBiome(accessor.seed, x, y, z, this), x.toDouble(), z.toDouble())
    }

    override fun getBiomeForNoiseGen(biomeX: Int, biomeY: Int, biomeZ: Int): Biome {
        val chunk = getChunk(biomeX shr 2, biomeZ shr 2)
        return chunk.biomeArray?.getBiomeForNoiseGen(biomeX, biomeY, biomeZ)
