...
public abstract class MixinTileEntityChest extends TileEntity implements ChestInfo, IParallelUpdate {
    ...
    public void updateParallel(@NotNull List<Runnable> async, @NotNull List<Runnable> callbacks) {
        ...
        if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + i + j + k) % 200 == 0) {
            ...
            async.add(() -> {
                List<EntityPlayer> list = this.world.getEntitiesWithinAABB(
                    EntityPlayer.class,
                    new AxisAlignedBB((float) i - 5.0F,
                        (float) j - 5.0F,
                        (float) k - 5.0F,
                        (float) (i + 1) + 5.0F,
                        (float) (j + 1) + 5.0F,
                ...
                callbacks.add(() -> {
                    for (EntityPlayer entityplayer : list) {
                        if (entityplayer.openContainer instanceof ContainerChest) {
                            ...
                            if (iinventory == this || iinventory instanceof InventoryLargeChest && ((InventoryLargeChest) iinventory).isPartOfLargeChest(
                                (IInventory) this)) {
        ...
        if (this.numPlayersUsing > 0 && this.lidAngle == 0.0F && this.adjacentChestZNeg == null && this.adjacentChestXNeg == null) {
            ...
            async.add(() -> this.world.playSound(
                null,
                finalD,
                (double) j + 0.5D,
                finalD1,
