...
public abstract class MixinFlowableFluid {
    ...
    @Shadow protected abstract boolean isFlowBlocked(BlockView world, BlockPos pos, Direction direction);
    ...
    public Vec3d getVelocity(BlockView world, BlockPos pos, FluidState state) {
        ...
        if (state.get(FALLING)) {
            for (int i = 0; i < HORIZONTAL_DIRECTIONS.length; i++) {
                ...
                if (this.isFlowBlocked(world, mutable1, direction)
                    || this.isFlowBlocked(world, mutable2.set(mutable1, Direction.UP), direction)) {
