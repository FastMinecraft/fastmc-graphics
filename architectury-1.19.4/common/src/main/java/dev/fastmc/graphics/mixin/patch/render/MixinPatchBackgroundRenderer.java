...
public class MixinPatchBackgroundRenderer {
    ...
    public static void render(Camera camera, float tickDelta, ClientWorld world, int i2, float f) {
        ...
        if (cameraSubmersionType == CameraSubmersionType.WATER) {
            ...
            int j2 = world.getBiome(camera.getBlockPos()).value().getWaterFogColor();
            ...
            float x = MathHelper.clamp(
                MathHelper.cos(world.getSkyAngle(tickDelta) * ((float) Math.PI * 2)) * 2.0f + 0.5f,
                0.0f,
                1.0f
            );
            ...
            Vec3d vec3d3 = CubicSampler.sampleColor(
                vec3d2,
                (i, j, k) -> world.getDimensionEffects().adjustFogColor(Vec3d.unpackRgb(biomeAccess.getBiomeForNoiseGen(
                    i,
                    j,
                    k
                ).value().getFogColor()), x)
            ...
            if (i2 >= 4) {
                ...
                Vector3f vec3f = new Vector3f(g, 0.0f, 0.0f);
                r = camera.getHorizontalPlane().dot(vec3f);
                if (r < 0.0f) {
                    r = 0.0f;
                }
                if (r > 0.0f && (fs = world.getDimensionEffects().getFogColorOverride(
